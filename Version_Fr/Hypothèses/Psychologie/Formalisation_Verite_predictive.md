...
## 1. Problématique : Le Piège du "Faux Vide" et la Définition de la Vérité
...
### 2.1. La Sensibilité au Mensonge ($\beta$)
Le paramètre $\beta$ (raideur de la fonction de cohérence) représente la **tolérance à l'incohérence** du système :
*   **$\beta$ élevé :** Systèmes très sensibles à la vérité (ex: profils logiques, TSA, recherche de cohérence stricte). Un mensonge même mineur ($\Delta > 0$) annule presque totalement l'effet du signal ($C \to 0$).
*   **$\beta$ faible :** Systèmes tolérants au déni ou au mensonge (ex: mécanismes de défense pathologiques). Le signal peut être efficace même avec une faible cohérence, mais le risque d'effondrement futur est accru.
...
| **Écart de Vérité ($\Delta$)** | **GSR (Conductance Cutanée)** | Le pic de conductance lors d'une affirmation indique un conflit cognitif (stress de mensonge). |
...
| **Cohérence ($C$)** | **Fonction de GSR** | Calculée comme l'inverse de la réponse de stress au mensonge. |
...
**Interprétation :**
1.  **Terme de Réduction :** Si le sujet dit une phrase avec un **GSR élevé** (mensonge détecté), le terme $C$ tend vers 0. La HRV ne s'améliore pas, voire diminue (stress accru).
2.  **Terme de Réduction (Vrai) :** Si le sujet dit une phrase avec un **GSR faible** (vérité perçue), le terme $C$ tend vers 1. La HRV augmente (homéostasie).
...
1.  **Diagnostic de $\beta$ :** Avant d'appliquer une "vérité brute", il faut évaluer la sensibilité du patient au mensonge.
    *   Pour un $\beta$ élevé : Approche purement factuelle, logique, sans adoucissement.
    *   Pour un $\beta$ faible : Approche progressive, construction de la confiance avant la vérité.
...
3.  **Multimodalité :** L'efficacité du signal $S$ est maximisée en combinant plusieurs canaux (voix + toucher + respiration) pour augmenter la probabilité que le signal soit perçu comme "vrai" par le système.

## 5. Conclusion

La **Vérité** dans le MRCC n'est pas une croyance, mais une **condition de stabilité physique**.
*   La "vérité brutale" n'est pas une punition, mais le seul moyen d'assurer que le terme de cohérence $C$ reste proche de 1.
